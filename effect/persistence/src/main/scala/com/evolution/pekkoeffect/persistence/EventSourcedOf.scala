...
import com.evolution.pekkoeffect.{ActorCtx, Envelope, Receive}
...
object EventSourcedOf {
  ...
  final private[EventSourcedOf] class Apply[F[_]](private val b: Boolean = true) extends AnyVal {
  ...
  implicit class EventSourcedOfOps[F[_], A](val self: EventSourcedOf[F, A]) extends AnyVal {
    ...
    def map[B](f: A => B)(implicit F: Functor[F]): EventSourcedOf[F, B] = { actorCtx =>
  ...
  implicit class EventSourcedOfRecoveryStartedOps[F[_], S, E, A](
    ...
    def convert[S1, E1, A1](sf: S => F[S1], s1f: S1 => F[S], ef: E => F[E1], e1f: E1 => F[E], af: A => Resource[F, A1])(
      implicit F: Monad[F],
  ...
  implicit class EventSourcedOfReceiveEnvelopeOps[F[_], S, E, C](
    ...
    def widen[S1 >: S, C1 >: C, E1 >: E](sf: Any => F[S], ef: Any => F[E], cf: Any => F[C])(implicit
      F: Monad[F],
    ): EventSourcedOf[F, Resource[F, RecoveryStarted[F, S1, E1, Receive[F, Envelope[C1], Boolean]]]] =
      self.map(_.map(_.widen(sf, ef, cf)))

    def typeless(sf: Any => F[S], ef: Any => F[E], cf: Any => F[C])(implicit
      F: Monad[F],
