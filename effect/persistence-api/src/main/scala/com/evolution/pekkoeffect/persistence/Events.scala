...
/** @param values
  *   inner Nel[A] will be persisted atomically, This applies some restrictions on persistence layer. Please do not
...
object Events {
  ...
  implicit def orderEvents[A](implicit A: Order[A]): Order[Events[A]] =
    Order.by(_.values)
