...
object ConHub {
  ...
  trait Send[A, M] extends ConnTypes[A, M] {
    ...
    /**
     * @param msg
     *   the message will be delivered to all matched connections whether it is a Local or Remote
     *   connection
     * @return
     *   list of local connections that matched the message
     */
    def !(msg: M): SR

    /**
     * @param msgs
     *   messages will be delivered to all matched connections whether it is a Local or Remote
     *   connection
     * @return
     *   list of local connections that matched the message
     */
  ...
  trait Manage[Id, A, M] {
     ...
     *   connection's Id
     * @param version
     *   operations against a previous version will be ignored
     * @param con
     *   new connection
     * @param send
     *   function, which will be used to forward messages for a particular connection
     */
    def update(
      id: Id,
      version: Version,
    ...
    /**
     * Disconnect a connection with instance tracking, to check later for successful disconnect or
     * failure
     * @param id
     *   connection's Id
     * @param version
     ...
     *   time to live for disconnected state
     */
    def disconnect(id: Id, version: Version, reconnectTimeout: FiniteDuration): Result

    /**
     * Remove a connection from registry, triggering eventual listeners. Can be called directly when
     * connection is not expected to reconnect, or for connections without instance tracking.
     * @param id
     *   connection's Id
     * @param version
     *   operations against a previous version will be ignored
