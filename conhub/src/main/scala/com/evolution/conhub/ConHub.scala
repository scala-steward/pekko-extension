...
object ConHub {
  ...
  trait Send[A, M] extends ConnTypes[A, M] {
    ...
    /** @param msg
      *   message will be delivered to all matched connections whether it is Local or Remote connection
      * @return
    ...
    /** @param msgs
      *   messages will be delivered to all matched connections whether it is Local or Remote connection
      * @return
      *   list of local connections that matched the message
      */
    def !(msgs: Nel[M]): SR
  ...
  trait Manage[Id, A, M] {
    ...
    /** @param id
      *   connection Id
      * @param version
      *   operations against previous version will be ignored
      * @param con
      *   new connection
      * @param send
      *   function which will be used to forward messages for particular connection
      */
    ...
    /** Remove a connection from registry, triggering eventual listeners. Can be called directly when connection is not
      * expected to reconnect, or for connections without instance tracking.
      * @param id
      *   connection Id
      * @param version
      *   operations against previous version will be ignored
